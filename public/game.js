...
function preload() {
  ...
  for (let i = 1; i <= 5; i++) {
    this.load.image('vetement' + i, 'assets/vetement' + i + '.png');
  }
}
...
function create() {
  ...
  walls.create(0, height / 2, 'wall').setOrigin(0, 0.5).setDisplaySize(20, height);
  walls.create(width, height / 2, 'wall').setOrigin(1, 0.5).setDisplaySize(20, height);
...
function update() {
  ...
  this.physics.overlap(player, fallingObjects, function (player, fallingObject) {
    score++;
...
function showGameOverUI() {
  ...
  container.innerHTML = `
    ...
    <button id="menu-btn" style="margin:10px; padding:10px 20px; font-size:16px;">Menu</button>
  `;
